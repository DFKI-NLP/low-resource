...
from low_resource.modules.offset_embedders import OffsetEmbedder
from low_resource.metrics import F1Measure


@Model.register("low_resource_relation_classifier")
class LowResourceRelationClassifier(Model):
    """
    ...
    def __init__(
        ...
        super(LowResourceRelationClassifier, self).__init__(vocab, regularizer)

        self.text_field_embedder = text_field_embedder
        self.num_classes = self.vocab.get_vocab_size("labels")
        self.text_encoder = text_encoder

        self.embedding_dropout = (torch.nn.Dropout(encoding_dropout)
                                  if embedding_dropout > 0 else lambda x: x)
        self.encoding_dropout = (torch.nn.Dropout(encoding_dropout)
