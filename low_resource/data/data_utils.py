...
def convert_json_to_jsonl(path_in, path_out=None, min_tokens=6, force_recreate=False):
    """
    create json line file from json file (has to contain an array of instances)
    """
    print(f'convert to jsonl {path_in} ...')
    if path_out is None:
        path_out = (path_in + 'l') if path_in.endswith('.json') else path_in + '.jsonl'
        ...
        with open(path_out, 'w') as out_f:
            for inst in data:
                if len(inst['token']) >= min_tokens:
                    out_f.write(json.dumps(to_iob1(inst)) + '\n')
                    n += 1
                else:
                    n_discarded += 1
